...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <html lang="en"><body><a className="skip" href="#content">Skip to content</a><header className="site-header"><Link className="wordmark" href="/" aria-label="Dovet home"><DovetMark size={22} />Dovet</Link><nav aria-label="Main navigation"><Link href="/how-it-works">How it works</Link><Link href="/demo">Demo</Link><Link href="/docs">Docs</Link>{sourceUrl ? <a href={sourceUrl}>Source</a> : null}<Link className="nav-action" href="/demo">Run the demo</Link></nav></header><main id="content">{children}</main><footer><div><span className="wordmark"><DovetMark size={18} />Dovet</span><p>Keep the work. Change the agent.</p></div><nav aria-label="Footer navigation"><Link href="/docs">Docs</Link><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link><Link href="/changelog">Changelog</Link></nav><p className="disclosure">Independent open-source project. Not affiliated with OpenAI or Amazon.</p></footer></body></html>;
}
