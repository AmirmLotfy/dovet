...
Status: **IN PROGRESS — opening visual accepted; final narration and edit remain evidence-gated.**
...
- Higgsfield balance after the bounded visual and voice work: 1,249.3 credits; observed aggregate
  change: 13.5 credits.
...
normal-speed listening review before the full batch. Every accepted take must use the same
`voice_id` and `voice_type`, pass the speech-duration and pause checks, and be regenerated by
rewriting the line when it misses its window. Audio is never time-stretched.
